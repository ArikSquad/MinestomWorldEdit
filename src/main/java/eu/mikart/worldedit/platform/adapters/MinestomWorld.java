...
public final class MinestomWorld extends AbstractWorld {
    ...
    @Override
    public int getMinY() {
        return MinecraftServer.getDimensionTypeRegistry().get(getWorld().getDimensionType()).minY();
    }

    @Override
    public int getMaxY() {
        DimensionType dimensionType = MinecraftServer.getDimensionTypeRegistry().get(getWorld().getDimensionType());
    ...
    public BlockState getBlock(@NotNull BlockVector3 position) {
        ...
        int stateId = this.getWorld().getBlock(MinestomAdapter.INSTANCE.asBlockPosition(position)).id();
